---
title: 'Mentor Work Sample for Statistical Inference: Simulations and Hypothesis Testing with R'
...
ggplot(simus, aes(x = simus)) +
        geom_histogram(aes(y=..density..), binwidth=0.2, fill="lightblue", color="black") +
        stat_function(fun = dnorm, 
                      args = list(mean = lambda^-1, sd = (lambda*sqrt(n))^-1),
                      size = 1, 
                      color="lightsalmon2") + 
        theme_minimal() +
        labs(title = "Figure 1: Sampling Distribution of Averages",
             subtitle = "(n = 40, num = 1000)",
             x = "simulation mean") + 
...
ggplot(ToothGrowth, aes(x = factor(dose), y = len)) +
        ...
        ggtitle("Figure 2: Guinea Pig Tooth Length versus Dose by Supplement Type") + 
        theme(plot.title = element_text(hjust = 0.5, vjust = 3, size = 14, face = "bold"))
```
...
```{r}
h0.1 <- t.test(data = ToothGrowth[ToothGrowth$dose %in% c("0.5", "1"),], 
               len ~ dose, paired = F, var.equal = F, alternative = "two.sided")
h0.1
```
...
```{r}
h0.2 <- t.test(data = ToothGrowth[ToothGrowth$dose %in% c("0.5", "2"),], 
               len ~ dose, paired = F, var.equal = F, alternative = "two.sided")
h0.2
```
...
```{r}
h0.3 <- t.test(data = ToothGrowth[ToothGrowth$dose %in% c("1", "2"),], 
               len ~ dose, paired = F, var.equal = F, alternative = "two.sided")
h0.3
```
...
```{r}
h1.1 <- t.test(data = ToothGrowth[ToothGrowth$dose %in% c("0.5"),], 
               len ~ supp, paired = F, var.equal = F, alternative = "two.sided")
h1.1
```
...
```{r}
h1.2 <- t.test(data = ToothGrowth[ToothGrowth$dose %in% c("1"),],
               len ~ supp, paired = F, var.equal = F, alternative = "two.sided")
h1.2
```
...
```{r}
h1.3 <- t.test(data = ToothGrowth[ToothGrowth$dose %in% c("2"),], 
               len ~ supp, paired = F, var.equal = F, alternative = "two.sided")
h1.3
```
