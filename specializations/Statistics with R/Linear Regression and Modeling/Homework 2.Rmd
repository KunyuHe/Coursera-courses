...
Yes, but not strongly recommended.
...
ggplot(data=mlb11, aes(x=hits, y=runs))+geom_point()+stat_smooth(method="lm", se=TRUE)
...
summary(m5)
mlb11%>%
  summarise(cor(runs,bat_avg), cor(runs, hits)) 
```
...
```{r diag}
# type your code for the Exercise here, and Knit
ggplot(data=mlb11, aes(x=new_obs, y=runs)) + geom_point()+stat_smooth(method="lm", se=TRUE)
ggplot(data=m6, aes(x=.fitted, y=.resid)) +geom_point()+geom_hline(yintercept=0, linetype="dashed")
ggplot(data = m6, aes(sample = .resid)) +
  stat_qq()
ggplot(data=m6, aes(x=.resid))+geom_histogram(binwidth=10)
```
