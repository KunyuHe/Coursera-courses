...
sfo_feb_flights %>% summarise(med=median(arr_delay), n=n())
```
...
- Then, we need to `arrange` these average delays in 
`desc`ending order
...
nycflights %>% summarise(ot_rate = sum(arr_type == "on time", dep_type == "delayed") / n())
```