...
ggplot(nc_1, aes(x = habit, y = weight)) +
  geom_boxplot()
```
...
Reject the null hypthesis with 95% confidence and conclude that we are 95% confident that the avergae weights of babies born to smoking and non-smoking mothers are different.
</div>
...
<ol>
<li> We are 95% confident that the average weights of babies born to non-smoking moms are 0.05 to 0.58 pounds higher than those of babies born to smoking moms. </li>
</ol>
```{r habit-weight-ci}
...
The 90% confidence interval is wider than that of 95% confidence.
</div>
```{r weeks-ci-90}
...
Fail to reject the null hypthesis with 95% confidence and conclude that under a significance level of 5%, the avergae weights gained during pregnancy of young and mature mothers are not different.
</div>
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
...
nc_2 <- nc%>%
  filter(mature == "younger mom" | mature =="mature mom")
ggplot(nc_2, aes(x = mature, y = mage)) +
  geom_boxplot()
```
