...
ggplot(samp, aes(x=area)) + geom_histogram(binwidth = 200)
```
...
90%£»
Less than the confidence level, maybe the size of stimulation is not large enough.
</div>
...
ci_2 <- ci_2 %>%
  mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))
ci_data_2 <- data.frame(ci_id_2 = c(1:50, 1:50),
                      ci_bounds_2 = c(ci_2$lower, ci_2$upper),
                      capture_mu_2 = c(ci_2$capture_mu, ci_2$capture_mu))
ggplot(data = ci_data_2, aes(x = ci_bounds_2, y = ci_id_2, 
                           group = ci_id_2, color = capture_mu_2)) +
  geom_point(size = 2) +  
  geom_line() +           
