...
m <- lm(data= ames_train, log_price ~ log_area + log_Lot.Area + Bedroom.AbvGr + Overall.Qual + Land.Slope)
AIC_m <- stepAIC(m, k = 2)
summary(AIC_m)
BIC_m <- stepAIC(m, k = log(1000))
summary(BIC_m)
```
...
m1 <- lm(data= ames_train, log_price ~ Bedroom.AbvGr)
summary(m1)
```
...
m2 <- lm(data= ames_train, log_price ~ log_area)
pred2 = predict(m2, newdata = ames_train)
ames_train$resid2 = resid(m2)
ames_train %>% 
  group_by(Neighborhood) %>% 
  summarise(mean_resid = mean(resid2)) %>% 
...
m4 <- lm(log_price ~ Overall.Qual + Bsmt.Qual + Garage.Qual, data = ames_train)
ames_no_na <- na.omit(ames_train[c("Overall.Qual", "Bsmt.Qual", "Garage.Qual", "log_price")])
```
...
m5 <- lm(data=ames_train, log_price ~ Overall.Cond + Overall.Qual) 
summary(m5)
pred5 = predict(m5, newdata = ames_train)
ames_train$pred5 = exp(pred5)
ames_train %>% group_by(MS.SubClass) %>% summarise(median_price=mean(pred5)) %>% arrange(desc(median_price))
```
...
m6 <- lm(data= ames_normal, log_price ~ log_area)
summary(m6)
summary(m2)
```
